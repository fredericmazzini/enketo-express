...
module.exports = function( grunt ) {
    ...
    grunt.registerTask( 'widgets', 'generate widget reference files', function() {
        ...
        var WIDGETS_SASS = 'app/views/styles/component/_widgets.scss';
        ...
        var content = PRE + '\'use strict\';\n\n' +
            `module.exports = [
        ${Object.keys(widgets).map(function(key){ 
            return 'require( \'' + ( coreWidgets[widgets[key]] || widgets[key] ) + '\' )';}).join(',\n\t')}\n];`;
        grunt.file.write( WIDGETS_JS, content );
        grunt.log.writeln( 'File ' + WIDGETS_JS + ' created' );
        content = PRE +
            Object.keys( widgets ).map( function( key ) {
                return '@import "' + ( coreWidgets[ widgets[ key ] ] || widgets[ key ] ) + '"';
            } ).join( ';\n\t' ) + ';';
